describe('Ver auditorios disponibles', () => {
    it('Debe visitar la pagina de auditorios disponibles', () => {
        ...
        cy.get("@token").then((token) => {
            cy.get("@user").then(user => {
                cy.visit('http://localhost:5173/espacios', {
                    onBeforeLoad(win) {
                ...
                cy.contains("Espacios Disponibles")
            })
        })
    });
});
